...
## 0. 最初にやること: 演習パックで手を動かす

**シェルスクリプトを書いた経験がない場合、いきなり案件No.1のスクリプト(約300行)を読むのは効率が良くありません。** 先に [exercises/](../exercises/README.md) の演習パック(全21演習・約18〜20時間)に取り組むことを推奨します。

| 進め方 | 内容 |
|---|---|
| 推奨 | 演習パックのステージNを終える → 対応する案件Noを読む、をステージごとに繰り返す |
| 最短 | 演習パックのステージ1(ex01〜ex06、約5時間)だけ終えてから案件No.1に入る |
| 経験者 | 演習パックの `./check.sh --list` で一覧を見て、自信がない演習だけ拾う |

演習パックのステージは案件No.1〜No.6に1対1で対応しています(ステージ1→案件No.1、ステージ2→案件No.2、以下同様)。演習で書いた部品が、案件のスクリプトの中でどう組み合わさっているかが分かる構成です。

演習パック全体の学習時間の目安は約18〜20時間で、これは案件パック(約41〜61時間)の**前**に置く投資です。先に演習を済ませることで案件パックの理解速度が上がるため、合計の学習時間はむしろ短くなることを狙っています。
...
|---|---|---|---|
| No.1 ユーザーアカウント自動化 | Linuxの基本操作(cd/ls/catなど)、権限(root/一般ユーザー)の概念 | ★★★ | [docs/04-environment-setup.md](04-environment-setup.md)のコマンド早見表を先に一読。加えて[演習パックのステージ1](../exercises/README.md)(ex01〜ex06)を先に終えると、案件のスクリプトがそのまま読める |
...
上記に加えて、事前の[演習パック](../exercises/README.md)に約18〜20時間を見込んでおくと安心です(演習パック → 案件パックの合計で約60〜80時間)。

はじめてLinuxサーバーに触れる場合は、上限に近い時間(週末中心で2〜3ヶ月程度)を見ておくと現実的です。詰まった箇所は各案件の`05-troubleshooting.md`を読むと、時間短縮につながります。

### 3.1 改善案件パックに進む場合の目安

構築案件(projects/)を一通り終えたら、[改善案件パック(improvements/)](../improvements/README.md)に進むと、「作れる」に「**すでにある運用を分析して直せる**」が加わります。面接で判断力を問われたときに答えられるようになるのは、こちらのパックです。

| 取り組み方 | 目安時間 |
|---|---|
| 読んで理解する(全6件) | 3〜6時間(1件30〜60分) |
| 検証環境で実際に再現する(全6件) | 約40〜70時間(1件6〜15時間) |

改善案件は、対応する構築案件を先に理解しているほど早く進みます(どの構築案件が前提になるかは[改善案件パックのREADME](../improvements/README.md)に対応表があります)。**全6件を読んだうえで、興味のある2〜3件だけ実際に手を動かして再現する**という進め方でも十分に効果があります。改善の型そのものは[improvements/docs/01-kaizen-method.md](../improvements/docs/01-kaizen-method.md)にまとまっているため、先にそれを読むと全体が短時間で頭に入ります。

## 4. このポートフォリオを終えた後に学ぶとよいこと

構築案件No.1〜No.6と[改善案件パック](../improvements/README.md)を一通り終えた後、次のステップとして学習を検討するとよい発展的なトピックを紹介します。いずれも、本ポートフォリオで扱った「自動化」「IaC」「CI/CD」「運用改善」の考え方の延長線上にあります。
