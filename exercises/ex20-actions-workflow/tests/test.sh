...
#-------------------------------------------------------------------------------
describe "test ジョブ: 配布前に実行する2つの検査"
#-------------------------------------------------------------------------------
# run: で1行書く形でも、run: | の下に並べる形でも合格するように、
# 行頭の run: は「あってもなくてもよい」形の正規表現で判定する。
# 行頭を [[:space:]]* に限定しているので、# で始まるコメント行には一致しない。
hint "test ジョブの1つ目の検査です。run: shellcheck scripts/*.sh と書いて、シェルスクリプトを静的解析します。"
assert_file_contains "$TARGET" \
    '^[[:space:]]*(run:[[:space:]]*)?shellcheck[[:space:]]+scripts/\*\.sh[[:space:]]*$' \
    "shellcheck scripts/*.sh で静的解析を実行する"

hint "test ジョブの2つ目の検査です。run: bash scripts/test.sh と書いて、ex21 で書くテストスクリプトを呼び出します。"
assert_file_contains "$TARGET" \
    '^[[:space:]]*(run:[[:space:]]*)?bash[[:space:]]+scripts/test\.sh[[:space:]]*$' \
    "bash scripts/test.sh でテストスクリプトを実行する"
