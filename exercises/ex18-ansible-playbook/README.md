...
```yaml
mode: "0644"     # 正しい。文字列として渡り、Ansibleが8進数として解釈する
mode: 644        # 危険。10進数の644として読み込まれ、まったく違う権限になる
```

パーミッションは8進数(`0644` = `rw-r--r--`)ですが、YAMLはクォートの無い数字を数値として読み込みます。`644` と書くと10進数の644(8進数では1204)になり、`--w----r-T` のような意図しない権限になってしまいます。

`0644` のように先頭に `0` を付ければ8進数として読まれますが、その解釈はYAMLのバージョンによって変わります。Ansibleの公式ドキュメントでも「先頭に0を付けるか、クォートで囲むか」が案内されており、**どちらでも確実な後者(クォート)を使う**のが安全です。**`mode` は必ずクォートで囲む**と覚えてください。
...
| No. | 条件 | 期待する動作 |
|---|---|---|
| 1 | 雛形の TODO | `TODO` を含む行が1行も残っていない(採点はこの1点だけを見ます。`TODO` を含まないコメント行は残してもかまいません) |
...
- `hosts:` `become:` `vars:` `tasks:` `handlers:` は play 直下のキー。行頭から半角スペース2つの深さに揃える
- タスクは `tasks:` よりさらに2スペース深い位置から `- ` で始める(`-` のあとに半角スペースを1つ)
- タスクの中身(モジュール名や `notify:`)は、`- ` の2文字ぶん右にずれた位置、つまり `name` と同じ深さに揃える
- モジュールの引数は、モジュール名よりさらに2スペース深くする
- `handlers:` は `tasks:` と同じ深さに書き、その下のハンドラはタスクとまったく同じ形で書く
...
| `could not find expected ':'` / `mapping values are not allowed in this context` | 前者は `hosts:webservers` のようにコロンの後ろの半角スペースを忘れている。`キー: 値` と1つ空ける。後者は値の中に半角コロンが入っている(`name: web: 01`)。値全体をダブルクォートで囲む |
...
| パーミッションが `--w----r-T` のような変な値になる | `mode: 644` のようにクォートも先頭の `0` も無く書いた。10進数の644として読まれるため。`mode: "0644"` と囲む |
