...
| 2. バックアップと定期実行 | 4 | 「作る・消す・知らせる・定時に動かす」という運用の1サイクルを体験する | 世代管理の必要性とドライランの価値を、自分の言葉で説明できる |
| 3. ログ監視と常駐化 | 4 | 実行と実行のあいだに情報を引き継ぐ「状態」の概念を導入する。常駐サービス化まで進む | 状態ファイルが必要な理由を説明できる |
| 4. 死活監視と状態管理 | 3 | 対象が複数になったときの設計と、誤検知(フラッピング)対策を学ぶ | 「1回の失敗で即通知」がなぜ良くないかを説明できる |
...
1日1時間なら約3週間、週末中心(土日に4時間ずつ)でも約3週間が目安です。これは案件パック(約41〜61時間)の**前**に置く投資であり、演習を先に済ませることで案件パックの理解速度が上がるため、総学習時間はむしろ短くなることを狙っています。
...
- 1演習あたり**14〜33項目**(多くは16〜18項目。ステージ1の総合演習ex06が最多の33項目)。正常系だけでなく、異常系(引数不足・ファイル無し・コマンド失敗)を必ず含める
...
| すべての雛形がテストに落ちる(課題として成立している) | `./check.sh` が終了ステータス1(CIでは参考情報として出力するだけで、失敗しても止めない。学習者が自分のフォークに解答をpushする使い方を妨げないため) |
| すべてのシェルスクリプトに構文エラーが無い | `./tools/lint.sh` の bash -n チェック |
| パック自身のコードが静的解析を通る | `./tools/lint.sh` の shellcheck チェック(警告0件) |
| README.md の演習一覧が meta.env と一致する | `./tools/make-index.sh --check` |
| 上記を継続的に保証する | GitHub Actions で、mainブランチへのpushとpull requestのたびに自動実行 |
...
1. **root権限を必要とする採点を書かない**(学習者の環境を選ばないため)。
   どうしても実行ユーザーによって結果が変わる場合は、判定を黙って通さず `skip` で理由を表示する。
   現在この例外に当たるのは2件だけで、いずれも `skip` として明示している。
   ex02(rootでは読み取り権限の判定が成立しない)と ex06(root権限チェックを持つ課題のため、
   一般ユーザーではアカウント作成部分を実行できない)
