...
```text
# 実行結果イメージ(ex02 を root で採点した場合)
  - [7] スキップ: root で採点しているため、読み取り権限のテストは省略します
  - [8] スキップ: root で採点しているため、読み取り権限のメッセージのテストは省略します
  結果: 14/14 合格 (スキップ 2件)
```

**スキップは不合格ではありません。** ただし、その項目が検証されていないことは事実です。ex02 は一般ユーザーで採点すると16項目すべてが判定されます。

```bash
$ id -u          # 0 と表示されたら root で作業している
```

**ex06 だけは逆で、root で採点したほうが項目数が増えます。** ex06 は「root権限チェック」そのものが課題に含まれるため、一般ユーザーで実行するとスクリプトが権限エラーで終了し、アカウント作成の処理まで到達できないからです。

```text
# 実行結果イメージ(ex06 を一般ユーザーで採点した場合)
  - [13] スキップ: アカウント作成の判定は一般ユーザーで採点しているため省略します (sudo ./check.sh 06 で全項目を判定できます)
  結果: 12/12 合格 (スキップ 4件)
```

ex06 の仕上げには `sudo ./check.sh 06` を実行してください。33項目すべてが判定されます。採点は使い捨ての一時ディレクトリの中で行われ、`useradd` などはダミーに差し替わるため、sudo を付けても本物のユーザーは作られません。
